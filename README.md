...
- **实时预览**: 时间轴可视化已标记的所有片段
- **智能兼容预览**: 默认优先直接预览原视频，遇到解码失败时自动切换兼容预览
- **不打扰进度提示**: 兼容预览转码与批量导出均支持右上角轻量百分比进度条
...
6. **兼容预览（按需）**: 若原视频无法直接预览，可点击「启用兼容预览」进行转码预览
7. **批量导出**: 点击「Export All Clips」选择输出文件夹，导出过程中会显示实时进度百分比

## ⚠️ 预览兼容说明

- 在 macOS 上，部分 `HEVC/H.265`（尤其 10-bit、4K 或带特殊轨道）在 Electron 中可能出现“有声音无画面”。
- 本项目采用“**原视频直播优先** + **失败自动回退兼容预览**”策略，避免对超大文件默认全量转码。
- 兼容预览仅用于播放体验，导出剪辑仍基于原始视频时间点处理。
